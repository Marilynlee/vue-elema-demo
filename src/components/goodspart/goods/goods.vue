    ...
    <div ref="menu">
      <ul class="menu-wrapper border-1px">
        ...
        >
          <imgText
            class="border-1px"
            :pros="{show:item.type>0,size:'12',type:item.type,bg:'bg',text:item.name}"
          ></imgText>
        </li>
      </ul>
    </div>
    <div class="foots-wrapper" ref="foods">
      <ul>
        <li v-for="(item,index) in goods" :key="index" class="food-list foodListHook">
          ...
          <ul>
            <li
              v-for="(food,num) in item.foods"
              class="food-item border-1px"
              :key="num"
              @click="checkFood(food,$event)"
            >
              <div class="icon">
...
export default {
  ...
  mounted() {
    /* mounted无法保证goods子组件完全渲染， this.$nextTick是promise的异步队列，
    可以获取更新后的数据，但是无法拿到渲染完后的dom高度。
    setTimeout则是下一个事件循环，即可保证dom渲染完成，所以可以正确获取dom高度 */
    this.$nextTick(() => {
      let that = this;
      setTimeout(function() {
        that._initScroll();
